...
const getComplementUser = async (user) => {
  ...
  for (const coin in groupCoins) {
    ...
    result[0].totalCoin.push({
      ...
      cantidad: total,
      imagen: groupCoins[coin][0].imagen,
      variacion: groupCoins[coin][0].variacion,
      abreviacion: groupCoins[coin][0].abreviacion,
      precio: groupCoins[coin][0].precio,
    });
  }
...
controller.createUser = async (req, res) => {
  ...
  if (
    data.correo &&
    data.correo.toString().trim() != '' &&
    data.clave &&
    data.clave.toString().trim() != '' &&
    data.nombre &&
    data.nombre.toString().trim() != ''
  ) {
    let query = `
        INSERT INTO users (name, country_origin, balance, email, password)
        VALUES ('${data.nombre.trim()}', ${data.pais || 1}, ${
      data.saldo || 0
    }, '${data.correo.trim()}', '${data.clave.trim()}')`;
...
const getHistoryUser = async (id_user) => {
  let query = `
    SELECT u.id, u.id_country AS id_pais, u.action AS id_accion,cc.variation as variacion,cc.imagen as imagen,cc.code as abreviacion, 
    cc.price_equivalent as precio,
    CASE
...
controller.getCountries = async (req, res) => {
  ...
  let { err, result } = await mysql.aQuery(query);
  if (err) {
    res.send({ success: false, error: err, data: [] });
  }
...
controller.buy = async (req, res) => {
  ...
  if (
    data.id_moneda &&
    data.id_moneda.toString().trim() != '' &&
    data.id_usuario &&
    data.id_usuario.toString().trim() != ''
  ) {
